...
export const InvoicesCard = ({ data }: { data?: any }) => {
    const navigate = useNavigate();

    // Default data mapping from invoiceStatus Neo API or fallback
    // Based on typical Neo API response for invoiceStatus
    ...
    return (
        <Card
            className="h-full border-none shadow-sm cursor-pointer hover:ring-1 hover:ring-primary/20 transition-all active:scale-[0.98]"
            onClick={() => navigate("/dashboard/detail/invoices")}
        >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-[13px] font-bold text-gray-900 uppercase tracking-tight">INVOICES</CardTitle>
            </CardHeader>
            <CardContent className="space-y-8 pt-2">
                ...
                <div className="space-y-3">
                    ...
                    <div className="h-5 w-full flex bg-[#d1d5db] rounded-sm overflow-hidden mt-2">
                        <div
                            className="h-full bg-[#ef6c00] transition-all duration-500"
                            style={{ width: `${unpaidRatio}%` }}
                        />
                        <div
                            className="h-full bg-[#d1d5db] transition-all duration-500"
